...
def Online_train_classifierEEGNet_incremental_KD(args_dict):
    ...
    if order == 1:
        ...
        if trial > 1: 
            load_model_path_encoder = os.path.join(result_save_subject_checkpointdir, 'best_model.pt')
            if not os.path.exists(load_model_path_encoder):
                load_model_path_encoder = os.path.join(result_save_subject_checkpointdir, 'online_model.pt')
