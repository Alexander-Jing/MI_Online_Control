...
from helpers.models import EEGNetTest, ConvEncoderResBN, ConvEncoderClsFea, ResEncoderfinetune, ConvEncoder3_ClsFeaTL, ConvEncoder3ResBN, ConvEncoder_OutputClsFeaTL
from helpers.brain_data import Offline_read_csv, brain_dataset, Online_read_csv, Online_read_csv_selection
...
def Online_train_classifierLM_incremental_KD(args_dict):
    ...
    if restore_file != 'None':
        ...
        if use_pretrain:
            ...
            encoder_to_use.freeze_features(unfreeze=unfreeze_encoder_online)  # We unfreeze the model parameters
    ...
    if order == 1:
        ...
        if trial > 1: 
            if not use_pretrain:
                load_model_path = os.path.join(result_save_subject_checkpointdir, 'best_model_encoder_output.pt')
                if not os.path.exists(load_model_path):
                    load_model_path = os.path.join(result_save_subject_checkpointdir, 'online_model_encoder_output.pt')
                encoder_to_use_output.load(filename=load_model_path)
                model_to_use = encoder_to_use_output.double()
                model = model_to_use.to(device)
            else:
                load_model_path_encoder = os.path.join(result_save_subject_checkpointdir, 'best_model_encoder.pt')
                load_model_path = os.path.join(result_save_subject_checkpointdir, 'best_model_encoder_output.pt')
                if not os.path.exists(load_model_path):
                    load_model_path_encoder = os.path.join(result_save_subject_checkpointdir, 'online_model_encoder.pt')
                    load_model_path = os.path.join(result_save_subject_checkpointdir, 'online_model_encoder_output.pt')
                
                encoder_to_use.load(filename=load_model_path_encoder)  
                encoder_to_use.freeze_features(unfreeze=unfreeze_encoder_online)
                encoder_to_use_output.load(filename=load_model_path)
                model_to_use = ResEncoderfinetune(encoder=encoder_to_use, encoder_output=encoder_to_use_output).double()
                model = model_to_use.to(device)
    ...
    if order == 4:
        # order 4: recognize the MI class online without online updating the model, this is for the offline analysis
        sub_val_feature_array = [data[0:-1,:]]
        sub_val_label_array = np.array([motor_class])
        group_val_set = brain_dataset(sub_val_feature_array, sub_val_label_array)
        cv_val_batch_size = 1
    ...
    if order == 2:
        ...
        if (trial) % update_wholeModel == 1:
            ...
            if trial > 1:
                if use_pretrain:
                    encoder_to_use.load(filename=os.path.join(result_save_subject_checkpointdir, 'best_model_encoder.pt'))
                    encoder_to_use_output.load(filename=os.path.join(result_save_subject_checkpointdir, 'best_model_encoder_output.pt'))
                    model_to_use = ResEncoderfinetune(encoder=encoder_to_use, encoder_output=encoder_to_use_output).double()
                else:
                    encoder_to_use_output.load(filename=os.path.join(result_save_subject_checkpointdir, 'best_model_encoder_output.pt'))
                    model_to_use = encoder_to_use_output.double()
            
            model_to_use = model_to_use.double()
            model = model_to_use.to(device)
        ...
        combined_feature_array = np.concatenate((combined_feature_array, train_list.reshape(-1, data_online_store.shape[-2], data_online_store.shape[-1])), axis=0)
        combined_label_array = np.concatenate((combined_label_array, train_label.reshape(-1)), axis=0).astype(np.int64)
        ...
        if (trial) % update_trial == 0:
            ...
            selected_indices = indices[0][-update_trial * samples_online:]  
        ...
        if (trial) % update_trial == 0:           
            ...
            if (trial) % update_wholeModel == 0:
                ...
                criterion = MultiClassFocalLoss(device=device, alpha=[0.1,0.45,0.45])
                optimizer = torch.optim.Adam(model.parameters(), lr=lr)
